...
use crate::{
    ...
    EmitModeParams, query_sequence_numbers,
};
...
use core::{
    cmp::{max, min}, result::Result::{Err, Ok}, sync::atomic::{AtomicBool, Ordering}, time::Duration
};
...
impl SubmissionWorker {
                    ...
                    match req.replay_protector() {
                        ReplayProtector::SequenceNumber(seq_num) => {
                            if cur.submitted_sequence_number_range.is_none() {
                                ...
                                let (start, end) = cur.submitted_sequence_number_range.as_mut().unwrap();
                                if *start > seq_num {
                                    *start = seq_num;
                                }
                                if *end < seq_num + 1 {
    ...
    async fn wait_for_account_txn_summaries(
        ...
        loop {
            ...
            match query_txn_summaries(self.client(), start_version_by_address.clone()).await {
                Ok((account_to_txn_summaries, ledger_timestamp)) => {
                    ...
                    for (account, txn_summaries) in account_to_txn_summaries {
                        for txn_summary in txn_summaries {
                            ...
                            if let Some(account_data) = self.account_data.get_mut(&account) {
                                if let Some((submitted_time, _expiration_time)) = account_data.submitted_replay_protectors.remove(&txn_summary.replay_protector) {
                                    sum_of_completion_timestamps_millis += submitted_time.elapsed().as_millis();
                                    num_committed += 1;
                                }
                                account_data.version_to_fetch_next = max(account_data.version_to_fetch_next, txn_summary.version + 1);
                            }
                        }
                    }
                    ...
                    for (account, _) in start_version_by_address {
                        if let Some(account_data) = self.account_data.get_mut(&account) {
                            account_data.submitted_replay_protectors.retain(|_, (_, expiration_time)| {
            ...
            let max_txn_expiration_ts_secs = self.account_data
                .iter()
                .map(|(_, data)| data.submitted_replay_protectors.values().map(|(_, expiration_time)| *expiration_time).max().unwrap())
                .max()
                .unwrap();
            if aptos_infallible::duration_since_epoch().as_secs() >= max_txn_expiration_ts_secs + 240 {
                sample!(
    ...
    async fn wait_for_account_sequence_numbers(
        ...
        let mut num_expired = 0;
        loop {
            let pending_addresses: Vec<_> = self.account_data.iter().flat_map(|(address, data)| {
                ...
                Ok((sequence_numbers, ledger_timestamp_secs)) => {
                    for (address, account_sequence_number) in sequence_numbers {
                        ...
                        latest_fetched_counts
                            .insert(address, account_sequence_number);
                        
                        if account_sequence_number >= account_data.submitted_sequence_number_range.unwrap().1 {
                            account_data.submitted_sequence_number_range = None;
                        }
                    }

                    // All the submitted transactions are either committed or expired
                    if self.account_data.iter().all(|(_, data)| data.submitted_replay_protectors.is_empty()) {
                        break;
                    }
                },
                Err(e) => {
        ...
        (num_committed, num_expired, sum_of_completion_timestamps_millis, latest_fetched_counts)
    }
    ...
    async fn wait_and_update_stats(
        ...
        let has_submitted_orderless_txns = self.account_data
                                                    .iter()
                                                    .any(|(_, data)| data
                                                                                    .submitted_replay_protectors
                                                                                    .keys()
                                                                                    .any(|rp| 
                                                                                            matches!(*rp, ReplayProtector::Nonce(_))
                                                                                        )
                                                    );
    ...
    fn update_account_seq_num(
        ...
        for account in self.accounts.iter_mut() {
            ...
            if let Some(pair) = self.account_data.get(&account.address()).and_then(|data| data.submitted_sequence_number_range) {
                pair
            } else {
                return;
            };
