...
use crate::state_store::state_update_ref_map::BatchedStateUpdateRefs;
    ...
    pub fn update(
        ...
        let smt_updates = updates
            ...
            .flat_map(|shard| {
                shard
                    .iter()
                    .sorted_by(|(k1, _u1), (k2, _u2)| k1.crypto_hash_ref().cmp(k2.crypto_hash_ref()))
...
impl LedgerStateSummary {
    ...
    pub fn update<'kv>(
        ...
        let last_checkpoint = if let Some(updates) = updates_for_last_checkpoint {
            self.latest
                .update(persisted, proof_reader, updates)?
        } else {
